...
public class Searcher {
    ...
    public void search(String queryString) throws IOException, ParseException, InvalidTokenOffsetsException {
        ...
        //sort
        SortField sortField = new SortField("date", SortField.Type.DOC, true);
        ...
        for (ScoreDoc scoreDoc : topDocs.scoreDocs) {
            ...
            if (desc != null) {
                TokenStream tokenStream = analyzer.tokenStream("description", new StringReader(desc));
                // TokenStream将查询出来的搞成片段，得到的是整个内容
                result = highlighter.getBestFragment(tokenStream, desc);
                if(result==null){
                    System.out.println("desc: "+doc.get("title"));
                }else {
